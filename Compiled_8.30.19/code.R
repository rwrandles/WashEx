...
setwd("C:/Users/Rohnin/Documents/R Stuff/WashEx/Compiled_8.30.19/") ## Set working directory to locId of WashEx folder
...
getReferral <- function(billId, biennium){
  ...
  if(!is.na(id)){
    comm = master_comms$longName[master_comms$commId == id]
  
    ifelse(length(comm) > 0, return(comm), return(NA))
  }
  else{ return(NA) }
}
...
for(term in biennium_list){
  ...
  year_spons <- year_spons %>% mutate(cong = length(seq(1889,substr(term,1,4),2)))
...
master_sponsors <- add_row(master_sponsors, repId = "GOV_15", cong = 64, firstName = "Jay", lastName = "Inslee", type = "gov", district = "0", party = "D", gender = "M", biennium = "2015-16")
master_sponsors <- add_row(master_sponsors, repId = "GOV_17", cong = 64, firstName = "Jay", lastName = "Inslee", type = "gov", district = "0", party = "D", gender = "M", biennium = "2017-18")

master_sponsors <- add_row(master_sponsors, repId = "PPL_15", cong = 64, firstName = "The", lastName = "People", type = "ppl", district = "0", party = "0", gender = "0", biennium = "2015-16")
master_sponsors <- add_row(master_sponsors, repId = "PPL_17", cong = 64, firstName = "The", lastName = "People", type = "ppl", district = "0", party = "0", gender = "0", biennium = "2017-18")
...
master_bills <- master_bills %>% select(billId, billType, billNumber, 
                                        ...
                                        biennium, legalTitle, introducedDate, sponsor, primeSponsorId, majSpons, longDescription, billUrl)
...
for(biennium in biennium_list){
  tableCopy_bi <- tableCopy[tableCopy$biennium == biennium,]
  
  duplicates <- dupnames %>% filter(biennium == biennium, name != "Rules")
  ...
  Hnames <- master_comms$name[substr(master_comms$longName,1,1) == "H" & master_comms$biennium == biennium & !(master_comms$name %in% duplicates$name) & master_comms$name != "Rules"]
  Snames <- master_comms$name[substr(master_comms$longName,1,1) == "S" & master_comms$biennium == biennium & !(master_comms$name %in% duplicates$name) & master_comms$name != "Rules"]
  Hacr <- master_comms$acronym[substr(master_comms$longName,1,1) == "H" & master_comms$biennium == biennium]
  Sacr <- master_comms$acronym[substr(master_comms$longName,1,1) == "S" & master_comms$biennium == biennium]
  
  for(name in Snames){
    code <- master_comms$commId[master_comms$name == name & master_comms$biennium == biennium]
    
    tableCopy_bi$locId[which(unlist(lapply(tableCopy_bi$historyLine, grepl, pattern = paste(c("eferred to ",
                                                                                                 "assed to ",
                                                                                                 "efer to "), name, "\\>", sep = "", collapse = "|"))))] = code
    tableCopy_bi$agency[which(unlist(lapply(tableCopy_bi$historyLine, grepl, pattern = paste(c("eferred to ",
                                                                                               "assed to ",
                                                                                               "efer to "), name, "\\>", sep = "", collapse = "|"))))] = "S"
  }
  for(name in Hnames){
    code <- master_comms$commId[master_comms$name == name & master_comms$biennium == biennium]
    ...
    tableCopy_bi$agency[which(unlist(lapply(tableCopy_bi$historyLine, grepl, pattern = paste(c("eferred to ",
                                                                                               "assed to ",
                                                                                               "efer to "), name, "\\>", sep = "", collapse = "|"))))] = "H"
  }
  
  for(acr in Sacr){
    tableCopy_bi$locId[which(unlist(lapply(tableCopy_bi$historyLine, grepl, pattern = paste(acr, "\\>", sep = ""))))] = master_comms$commId[master_comms$acronym == acr & master_comms$biennium == biennium]
  ...
  for(name in duplicates$name){
    lines <- which(unlist(lapply(tableCopy_bi$historyLine, grepl, pattern = paste(c("eferred to ",
                                                                                    "assed to ",
                                                                                    "efer to "), name, "\\>", sep = "", collapse = "|"))))
    
    tableCopy_bi$locId[which(!is.na(tableCopy_bi$agency))[which(!is.na(tableCopy_bi$agency)) %in% lines]] = 
      (master_comms %>% filter(name == name, biennium == biennium))$commId[match(tableCopy_bi$agency[which(!is.na(tableCopy_bi$agency))[which(!is.na(tableCopy_bi$agency)) %in% lines]], substr((master_comms %>% filter(name == name, biennium == biennium))$longName,1,1))]
...
for(i in 1:nrow(master_bills)){
  info <- memInfor(master_bills$biennium[i], master_bills$firstRef[i], master_bills$primeSponsorId[i])
