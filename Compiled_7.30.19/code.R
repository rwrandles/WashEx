...
setwd("C:/Users/Rohnin/Documents/R Stuff/WashEx_7_30_19/") ## Set working directory to location of WashEx folder
...
getCommittees <- function(biennium){
  path <- paste("CommitteeService.asmx/GetCommittee?biennium=",
                biennium, sep = "")
...
## Sort legislators by last name, and give each entry a unique numberical Id
...
for (biennium in biennium_list){
  ...
  for (d in descriptions){ #d is a single line of an event in the bill's history
      ...
      for (committee in committee_names){
        ...
        if (location_flag == "House"){
          ...
          if (grepl("Third reading|Third Reading", d)){
            actionTableBiennium$locId[counter] = "House Floor" }
        ...
        if (location_flag == "Senate"){
          ...
          if (grepl("hird reading|Third Reading", d)){
            actionTableBiennium$locId[counter] = "Senate Floor" }
    ...
    if (grepl("Delivered to Governor", d)){
      ...
      actionTableBiennium$locId[counter] = "Governor's Desk" }
    
    if (grepl("Governor signed", d)){
      #if true, then bill was signed
      actionTableBiennium$locId[counter] = "Sign" }
    ...
    if (grepl("onference", d)){
      actionTableBiennium$locId[counter] = "Conference" }
    
    if (grepl("Senate Rules", d)){
      actionTableBiennium$locId[counter] = subset_bi_df$CommitteeId[which(subset_bi_df$LongName==paste(location_flag, "Committee on Rules", sep = " "))]  }
    
    if (grepl("ffective date", d)){
      actionTableBiennium$locId[counter] = "Became law" }
...
datesTable <- read.csv("C:/Users/Rohnine")
...
namesTable <- master_sponsors %>% filter(FirstName != "")
namesTable <- unique(namesTable[, c(2,4,5)])
namesTable$firstYear <- rep(NA, nrow(namesTable))

for(name in 1:nrow(namesTable)){
  namesTable$firstYear[name] <- master_sponsors$Biennium[master_sponsors$repId == namesTable$repId[name]][1]
}

namesTable <- namesTable %>% select(repId, firstYear, FirstName, LastName)


membersTable <- data.frame()

for(committee in 1:nrow(master_comms)){
  if(!(master_comms$Name[committee] == "Juvenile Justice" | 
     master_comms$Name[committee] == "Select Committee on Elections" |
     (master_comms$LongName[committee] == "Senate Committee on Financial Institutions & Insurance" & master_comms$Biennium[committee] == "2005-06") |
     (master_comms$Name[committee] == "Health Care & Wellness" & master_comms$Biennium[committee] == "2005-06") |
     (master_comms$Name[committee] == "Higher Education" & master_comms$Biennium[committee] == "2005-06"))){
  
  currList <- getMembers(master_comms$Biennium[committee], strsplit(master_comms$LongName[committee], " ")[[1]][1], master_comms$Name[committee])
  currList <- currList %>% mutate(Biennium = master_comms$Biennium[committee], Committee = master_comms$LongName[committee]) %>% select(Biennium, Committee, Name)
  
  membersTable = rbind(membersTable, currList)
  }
}
